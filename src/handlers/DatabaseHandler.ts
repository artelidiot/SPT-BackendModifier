...
export class GlobalHandler {
    ...
    public handle(logger: ILogger, database: IDatabaseTables): void {
        ...
        logger.debug(`[backend-modifier] set 'Stamina.AimDrainRate' to: ${database.globals.config.Stamina.AimDrainRate}`)
        ...
        // Iterate over every quest in the game
        for (const [questId, quest] of Object.entries(database.templates.quests)) {
            // Iterate over the starting quest conditions
            for (const [conditionId, condition] of Object.entries(quest.conditions?.AvailableForStart)) {
                // Set the quest to be available immediately
                condition.availableAfter = 0
                logger.debug(`[backend-modifier] set availableAfter to ${condition.availableAfter} for quest: ${questId} in condition: ${conditionId}`)
            }
        }
    }
}